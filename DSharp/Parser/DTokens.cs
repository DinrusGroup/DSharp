...
namespace DSharp.Parser
{
    public class DTokens
    {
        ...
        public static string GetDescription(string token)
        {
			...
			{
				if (token == "@disable")
					return @"Отключает декларацию
Обращение к декларации, помеченной атрибутом @dis­able вызывает ошибку при компиляции. 

Используется для явного отключения некоторых операций 
или перегрузок при компиляции, 
чтобы не возникало ошибок времени выполнения.";

				if (token == "@property")
					return 
@"Функции-свойства 
могут вызываться без скобок (работая как свойства).
...
void foo() {
  ...
  bar(s.x);  // calls bar(s.x())
				...
				if (token == "@safe")
					return @"Безопасные функции

В безопасных функциях недопустимы следующие операции:

- Приведение из типа указатель 
  в любой тип, кроме void*.
- Приведение из любого неуказательного типа 
  в указательный тип.
...
"- Are co­vari­ant with safe or sys­tem func­tions");
        ...
        public static string GetDescription(byte token)
        {
            switch (token)
            {
                ...
                case Switch:
                    return "switch(a)\n{\n   case 1:\n      foo();\n      break;\n   case 2:\n      bar();\n      break;\n   default:\n      break;\n}";
                default: return "Ключевое слово D";
            }
        }
