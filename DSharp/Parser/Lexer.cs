...
namespace DSharp.Parser
{
	public sealed class Lexer : IDisposable
	{
		...
		/// <asummary>
		/// используется для оригинального значения строк (с искейп-последовательностями).
		/// </asummary>
		...
		/// <summary>
		/// Временное хранилище для коментариев DDoc
		/// </summary>
		...
		/// <summary>
		/// Следующий DToken (Вызов текущего токена <see cref="CurrentToken"/> после следующего <see cref="NextToken"/>) .
		/// </summary>
		...
		/// <summary>
		/// Вызывается перед операцией peek.
		/// </summary>
		...
		/// <summary>
		/// Возвращает следующий токен. Второй вызов Peek() даёт следующий токен после последнего вызова Peek() и т.д.
		/// </summary>
		/// <returns>Объект <see cref="CurrentToken"/>.</returns>
		public DToken Peek()
		{
			...
			while ((nextChar = ReaderRead()) != -1)
			{
				...
				switch (nextChar)
				{
					...
					case '\\':
						...
						OnError(y, x, "Строки искейп-последовательности депрекированы!");
						break;
					case '\'':
						token = ReadChar();
						break;
							...
							if (canBeKeyword)
							{
								...
								if (s.Length >= 8 && s[0] == '_' && s[1] == '_')
								{
									...
									if (s == "__VENDOR__")
									{
										...
										literalValue = "Парсер D v" + System.Reflection.Assembly.GetExecutingAssembly().GetName().Version.ToString(3) + " Виталия Кулич";
									}
				...
				{
					OnError(Line, Col, "Неверный символ");
					//StopLexing();
					break;
				}
			}
		...
		string ReadIdent(char ch, out bool canBeKeyword)
		{
			...
			while (true)
			{
				if (ch == '\\')
				{
					...
					if (peek != 'u' && peek != 'U')
					{
						OnError(Line, Col, "В идентификаторах могут находиться только искейп-последовательности Юникода");
					}
					canBeKeyword = false;
					...
					if (surrogatePair != null)
					{
						...
						{
							OnError(Line, Col, "В идентификаторах искейп-последовательности Юникод не могут использоваться для представления неверных символов");
						}
				...
				{
					OnError(Line, Col, String.Format("Слишком длинный идентификатор"));
		...
		DToken ReadDigit(char ch, int x)
		{
			...
			{
				OnError(Line, x, "Цифровые литералы могут начинаться только с цифр (0-9) или точки ('.')!");
				return null;
			}
		...
		DToken ReadString(int initialChar)
		{
			...
			if (!doneNormally)
			{
				OnError(y, x, String.Format("Внутри строкового литерала достигнут конец файла"));
			}
		...
		DToken ReadVerbatimString(int EndingChar)
		{
			...
			if (nextChar == -1)
			{
				OnError(y, x, String.Format("Внутри строкового литерала verbatim достигнут конец файла"));
			}
		...
		/// <summary>
		/// читает искейп-последовательность
		/// </summary>
		...
		string ReadEscapeSequence(out char ch, out string surrogatePair)
		{
			...
			if (nextChar == -1)
			{
				OnError(Line, Col, String.Format("Конец файла достигнут внутри искейп-последовательности"));
				ch = '\0';
				return String.Empty;
			}
			int number;
			...
			switch (c)
			{
				...
				case 'x':
					...
					if (number < 0)
					{
						OnError(Line, Col - 1, String.Format("Неверная буква в литерале : {0}", c));
					}
					for (int i = 0; i < 3; ++i)
					{
				...
				case 'U':
					...
					for (int i = 0; i < 8; ++i)
					{
						...
						{
							OnError(Line, Col - 1, String.Format("Неверная буква в литерале : {0}", (char)ReaderPeek()));
							break;
						}
					}
					if (number > 0xffff)
				...
				case '&':
					...
					while (true)
					{
						...
						if (nextChar < 0)
						{
							OnError(Line, Col - 1, "КФ достигнут внутри именованной символьной сущности");
							ch = '\0';
							return string.Empty;
						}
						...
						{
							OnError(Line, Col - 1, "В именованной символьной сущности обнаружен неожиданный символ: " + c);
							ch = '\0';
							return string.Empty;
						}
					}
					...
					{
						OnError(Line, Col - 1, "Пустые именованные символьные сущности недопустимы");
						ch = '\0';
						return string.Empty;
					}
				...
				default:
					...
					{
						OnError(Line, Col, String.Format("Неожиданная искейп-последовательность : {0}", c));
						ch = '\0';
					}
					break;
			}
		...
		DToken ReadChar()
		{
			...
			if (nextChar == -1)
			{
				OnError(y, x, String.Format("Внутри символьного литерала достигнут конец файла"));
				return null;
			}
			char ch = (char)nextChar;
			char chValue = ch;
			...
			unchecked
			{
				if ((char)ReaderRead() != '\'')
				{
					OnError(y, x, String.Format("Знак незавершён"));
				}
			}
		...
		void ReadComment()
		{
			switch (ReaderRead())
			{
				...
				case '/':
					...
					break;
				default:
					OnError(Line, Col, String.Format("Ошибка при прочтении комментария"));
					break;
			}
		}
		...
		void ReadMultiLineComment(Comment.Type commentType, bool isNestingComment)
		{
			...
			OnError(Line, Col, String.Format("Достигнут КФ ранее конца многострочного комментария"));
		}
		...
		void ReadSpecialTokenSequence()
		{
			...
			if (cmd == "line") {
				...
				if (Line != startLine) {
					LexerErrors.Add (new ParserError (false, "Как минимум, нужен номер строки для директивы #line", DTokens.INVALID, new CodeLocation (x, Line)));
					return;
				}
				...
				LexerErrors.Add (new ParserError (false, "Неверная последовательность специальных токенов", DTokens.INVALID, new CodeLocation (x, Line)));
